...
class Zadanie:
    def __init__(self, n = 120, M = 5, N = 28):
    ...
    def mierz_czas(self, metoda, k=None):
        ...
        self.lista = []
        czas = 0.0
        Maciuś= uklad.Uklad(k) 
        for _ in range(self.M):
            Maciuś.losuj_uklad_symetryczny_dodatnio_okreslony()

            GJ=gaussjordan.GaussJordan(Maciuś)
            BN=banachiewicz.Banachiewicz(Maciuś)
            ...
            if metoda == 1:
                ...
                stoper = time.time()
                BN.rozklad()
                BN.rozwiaz_trojkatny_dolny()
                BN.rozwiaz_trojkatny_gorny()
                stoper = time.time() - stoper
            czas = czas + stoper
        # return czas/self.M
